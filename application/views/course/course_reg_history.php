                            ...
                            <div class="card">
                                <div class="card-body">
									<table
										...
										<tbody>
											<?php $i = 1; foreach($course_reg_history as $row){ ?>
											<tr>
												<td><?php echo $i++; ?></td>
												...
												<td>
                                                    <?php 
                                                        echo ($_SESSION['course_reg_close_date'] < date('m-d-Y')) ? "OPEN" : "CLOSED"; 
                                                    ?>
                                                </td>
                                                <td class="text-warning">
													<span>
													    <?php if($_SESSION['add_drop_deadline'] and $_SESSION['active_session'] == $row->value){ ?>
                                                        <a href="<?php echo site_url('course/ammend/'.md5(time().rand()).'/'.$row->id)?>"  class="text-info">
                                                            <i class="fa fa-plus-square"></i> Add/drop
                                                        </a>
                                                        <?php }else{ ?>
                                                        <a href="#"  class="text-danger">
                                                            <i class="fas fa-calendar-check"></i> CLOSED
                                                        </a>
                                                        <?php } ?>
													</span>
												</td>
